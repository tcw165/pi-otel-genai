...
[ ] <p0> span_manager.ts: apply PayloadPolicy/Redactor to span attributes — gen_ai.prompt, gen_ai.completion, gen_ai.tool.input, gen_ai.tool.output are set raw; privacy policy is unenforced
...
[ ] <p1> span_manager.ts onTurnEnd: call turn.flush() — method only null-checks; turn spans stay open until agent ends, making turn duration histograms incorrect
...
[ ] <p3> README.md: remove npm run typecheck (script does not exist) or add it to package.json
[ ] <p3> README.md: fix npm install → pnpm install in Development section
[ ] <p3> README.md: align package name (@oh-my-goose/pi-otel-genai) with install instructions
